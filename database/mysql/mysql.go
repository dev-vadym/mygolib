...
func NewDB(config DBConfig) DB {
	log := logger.Use.WithField("db", "MySQL")
	log.Info("Initialize the connection to the database...")
...
func (db *DB) Query(ar *ActiveRecord) (rs *ResultSet, err error) {
	...
	if results == nil || len(results) == 0 {
		...
		var scans []interface{}
		scans = GetX(scans, uint64(len(cols)), func() interface{} {
...
func (ar *ActiveRecord) Wheres(where map[string]interface{}) *ActiveRecord {
	if len(where) > 0 {
